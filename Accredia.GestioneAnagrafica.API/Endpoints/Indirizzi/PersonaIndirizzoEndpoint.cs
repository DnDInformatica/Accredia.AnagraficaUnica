...
public class PersonaIndirizzoEndpoint : ICarterModule
{
    public void AddRoutes(IEndpointRouteBuilder app)
    {
        ...
        app.MapGet("/api/persone/{EntitaAziendaleId}/indirizzi", async (
            [FromRoute] int EntitaAziendaleId,
            [FromServices] PersoneDbContext context) =>
        {
            var personaExists = await context.Set<Persona>()
                .AnyAsync(p => p.EntitaAziendaleId == EntitaAziendaleId && p.DataCancellazione == null);
            ...
            var indirizzi = await context.Set<PersonaIndirizzo>()
                .Where(pi => pi.EntitaAziendaleId == EntitaAziendaleId && pi.DataCancellazione == null)
                .Select(pi => new PersonaIndirizzoDTO.Response
                {
                    PersonaIndirizzoId = pi.PersonaIndirizzoId,
                    EntitaAziendaleId = pi.EntitaAziendaleId,
                    IndirizzoId = pi.IndirizzoId,
                    TipoIndirizzoId = pi.TipoIndirizzoId ?? 0,
                    Attivo = pi.Attivo,
        ...
        app.MapPost("/api/persone/{EntitaAziendaleId}/indirizzi", async (
            [FromRoute] int EntitaAziendaleId,
        ...
        {
            // Override EntitaAziendaleId dal route
            request.EntitaAziendaleId = EntitaAziendaleId;
            ...
            var personaExists = await context.Set<Persona>()
                .AnyAsync(p => p.EntitaAziendaleId == EntitaAziendaleId && p.DataCancellazione == null, cancellationToken);
            ...
            var exists = await context.Set<PersonaIndirizzo>()
                .AnyAsync(pi => pi.EntitaAziendaleId == EntitaAziendaleId 
            ...
            try
            {
                ...
                return Results.Created($"/api/persone/{EntitaAziendaleId}/indirizzi", response);
            }
            catch (Exception ex)
            {
        ...
        app.MapPut("/api/persone/{EntitaAziendaleId}/indirizzi/{personaIndirizzoId}", async (
            [FromRoute] int EntitaAziendaleId,
            ...
            CancellationToken cancellationToken) =>
        {
            ...
            var personaIndirizzo = await context.Set<PersonaIndirizzo>()
                .FirstOrDefaultAsync(pi => pi.PersonaIndirizzoId == personaIndirizzoId 
                    && pi.EntitaAziendaleId == EntitaAziendaleId 
                    && pi.DataCancellazione == null, cancellationToken);
        ...
        app.MapDelete("/api/persone/{EntitaAziendaleId}/indirizzi/{personaIndirizzoId}", async (
            [FromRoute] int EntitaAziendaleId,
            [FromRoute] int personaIndirizzoId,
            [FromServices] PersoneDbContext context,
            CancellationToken cancellationToken) =>
